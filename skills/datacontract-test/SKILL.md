...
description: >-
  ...
  with different semantics: output-port contracts under `models/output_ports/**/*.odcs.yaml`
  (tested against this project's Snowflake — "am I still producing what I promised?") and
  input-port contracts under `models/input_ports/*.odcs.yaml` (tested against the upstream
  Snowflake server — "is upstream still producing what I trusted?"). Trigger when the user
  asks to "test the data contracts", "verify the data product matches its contract", "are we
  still contract-conformant", "check upstream drift", or "run the contract tests".
---
...
- **Output-port contracts** at `models/output_ports/v<N>/*.odcs.yaml` — what this data product commits to produce. They test against **this project's** Snowflake target. A failure means we are no longer producing what we promised.
...
> 1. Pre-checks: confirm `datacontract` is on PATH and Snowflake credentials are in the environment.
